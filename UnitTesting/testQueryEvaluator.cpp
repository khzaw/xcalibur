...
void QueryEvaluatorTest::testSolveForSuchThatFollows(){
	...
	QueryTree* query0 = new QueryTree(follows0);

	vector<vector<int>> actualResult0 = qe->solveForSuchThatFollows(&table1, query0); // stmt s1; Select s1 such that Follows(s1, s1) | Expected <> (none)

	CPPUNIT_ASSERT(actualResult0[0].empty());

	QTNode* follows1 = new QTNode("Follows");
	QTNode* first1 = new QTNode("s1");
	QTNode* second1 = new QTNode(3);
	first1->setParent(follows1);
	follows1->addChild(first1);
	second1->setParent(follows1);
	follows1->addChild(second1);
	...
	vector<vector<int>> actualResult1 = qe->solveForSuchThatFollows(&table1, query1); // stmt s1; Select s1 such that Follows(1, s1) | Expected <<1>,<2>>
	/* for debugging
	if (!actualResult1.empty()){
		cout << endl;
		for (int i=0; i<actualResult1[0].size(); i++){
			cout << actualResult1[0][i] << " | " << actualResult1[1][i] << endl;	
		}
	}
	*/
...
void QueryEvaluatorTest::testSolveForSuchThatModifies(){
	...
	QTNode* follows0 = new QTNode("Modifies");
	QTNode* first0 = new QTNode("s1");
	QTNode* second0 = new QTNode("s1");
	first0->setParent(follows0);
	follows0->addChild(first0);
	...
	vector<vector<int>> actualResult0 = qe->solveForSuchThatModifies(&table1, query0); // stmt s1; Select s1 such that Modifies(s1, a1) | Expected <> (none)
	if (!actualResult0.empty()){
		cout << endl;
		for (int i=0; i<actualResult0[0].size(); i++){
			cout << actualResult0[0][i] << " | " << actualResult0[1][i] << endl;	
		}
	}
	CPPUNIT_ASSERT(actualResult0.size()==2);
}
